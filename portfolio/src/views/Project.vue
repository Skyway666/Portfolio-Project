<template>
    <div class="background">
        ...
        <div class="graphics-section">
            <div
                v-for="(layoutMediaItem, index) in project.layoutMedia"
                :key="index"
                class="graphic-wrapper"
            >
                <img
                    v-if="layoutMediaItem.type == 'IMAGE'"
                    :src="layoutMediaItem.src"
                    class="graphic"
                />
                <iframe
                    v-if="layoutMediaItem.type == 'VIDEO'"
                    :src="layoutMediaItem.src"
                    class="graphic video"
        ...
        <div class="text-section">
            ...
            <div class="text-section-links">
                <sui-button
                    v-for="(linkButton, index) in project.linkButtons"
                    @click="openLink(linkButton.link)"
                    :key="index"
                >
                    {{ linkButton.text }}
                </sui-button>
            ...
            <!-- Contributions -->
            <div
                v-if="project.contributions.length != 0"
        ...
        <div class="navigation">
            <sui-button
                ...
                icon="left arrow"
                @click="
                    $router.push('/project/' + getProjectName(projectIndex - 1))
                "
            />
            <sui-button
                v-if="projectIndex != $store.getters.projects.length - 1"
        ...
        computed: {
            project() {
                return this.$store.getters.projects.find(
                    (project) => project.name === this.$route.params.name
                );
            },
            projectIndex() {
                return this.$store.getters.projects.findIndex(
                    (project) => project.name === this.$route.params.name
                );
            },
        },
